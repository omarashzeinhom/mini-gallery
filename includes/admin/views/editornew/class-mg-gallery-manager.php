...
class MG_Admin_Integration
{
    ...
    public function enqueue_scripts($hook)
    {
        ...
        // Corrected JS path
        $js_path = MG_PLUGIN_PATH . 'includes/admin/views/editornew/assets/js/visual-editor.js';
        if (file_exists($js_path)) {
            wp_enqueue_script(
                'mg-visual-editor',
                MG_PLUGIN_URL . 'includes/admin/views/editornew/assets/js/visual-editor.js',
                array('jquery', 'wp-util'),
                filemtime($js_path), // Use filemtime for versioning
                true
            );
            ...
            error_log('MG Visual Editor JS not found: ' . $js_path);
        }

        // Corrected CSS path
        $css_path = MG_PLUGIN_PATH . 'includes/admin/views/editornew/assets/css/visual-editor.css';
        if (file_exists($css_path)) {
            wp_enqueue_style(
                'mg-visual-editor',
                MG_PLUGIN_URL . 'includes/admin/views/editornew/assets/css/visual-editor.css',
                array(),
                filemtime($css_path) // Use filemtime for versioning
            );
        } else {
            error_log('MG Visual Editor CSS not found: ' . $css_path);
        }
    }
